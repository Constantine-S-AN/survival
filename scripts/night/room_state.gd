...
static func from_dictionary(room_data: Dictionary, type_data: Dictionary, target_floor_id: String, target_floor_index: int) -> RoomState:
	...
	state.room_type_label = String(type_data.get("label", state.room_type_id.capitalize())).strip_edges()
	state.label = String(
		room_data.get("label", room_data.get("title", type_data.get("room_label", state.room_type_label)))
	).strip_edges()

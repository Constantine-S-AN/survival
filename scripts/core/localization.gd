...
const UI_TEXT := {
	...
	"meta.hub.guide_body_day1": {"en": "- Goal: till and plant at least two plots so Day 3 opens with a harvest.\n- Goal: turn today's pantry into your first lunch win with Field Stew or Herb Tea.\n- Push to Evening, launch night combat from the dock, and bring anything useful back for tomorrow.", "zh_CN": "- 目标：至少翻好并种下两块地，让第 3 天一开场就有收成。\n- 目标：用田里的库存做出第一波午市成功，优先尝试田园炖菜或香草茶。\n- 推进到傍晚后，从码头出发打第一场夜战，把任何有用的东西带回明天。"},
	"meta.hub.guide_body_day2": {"en": "- Water yesterday's crops before the rest of the route pulls you away.\n- A restaurant shift costs {value} actions, so lock the menu early if you want your first dependable lunch service.\n- Night haul matters now: spend it on unlock progress, premium dishes, or the next seed line instead of leaving it in storage.", "zh_CN": "- 在被其他路线分心前，先给昨天种下的作物浇水。\n- 一次餐馆营业会消耗 {value} 次行动，所以想打出第一轮稳定午市，就要尽早定好菜单。\n- 现在夜间战利品已经很关键：优先把它们用在解锁进度、高价菜，或下一条种子路线，而不是一直堆在仓库里。"},
	"meta.hub.guide_body_day3": {"en": "- Harvest first, then decide what becomes pantry stock, shop cash, or plated dishes.\n- Reef Salt, Glow Kelp, and Abyssfin are your first valuable night returns because they unlock stronger daytime payoffs.\n- Buy only the next seed or upgrade that still leaves room for service and tonight's departure.", "zh_CN": "- 先收成，再决定哪些进厨房、哪些拿去卖、哪些直接变成今天的菜。\n- Reef Salt、Glow Kelp 和 Abyssfin 是第一批真正值钱的夜间回报，因为它们会解锁更强的白天收益。\n- 只购买那些不会挤掉营业资金和今晚出发余量的下一步种子或升级。"},
